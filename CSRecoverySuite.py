...
def generateSamplingMask(shape, delta, saType):
  # Check delta is valid
  if(delta<=0.0)and(delta > 1.0):
    print('ERROR: Invalid undersampling ratio delta in generateSamplingMask.')
    sys.exit(-1)
  if(saType == 'bernoulli'):
    omega = np.where(np.random.uniform(0, 1, shape) < delta, True, False)
  elif(saType == 'triangle'):
    omega = VardensTriangleSampling(shape, delta)
  elif(saType == 'gaussian'):
    omega = VardensGaussianSampling(shape, delta)
  elif(saType == 'exponential'):
    omega = VardensExponentialSampling(shape, delta)
  else:
